...
const columns: ColumnDef<Tenant, unknown>[] = [
  {
    ...
    header: 'Name',
    cell: ({ row }) => (
      <Link href={`/tenants/${row.original.id}`} className="text-primary hover:underline font-medium">
        {row.original.name}
      </Link>
    ),
  },
  {
    ...
    header: 'Realm',
    cell: ({ row }) => <span className="font-mono text-xs">{row.original.realmName}</span>,
  },
  {
  ...
  },
  {
    ...
    header: 'Admin',
    cell: ({ row }) => <span className="text-sm">{row.original.adminEmail}</span>,
  },
  {
    id: 'users',
    header: 'Users',
    cell: ({ row }) => <span className="text-sm">{row.original.currentUsers}/{row.original.maxUsers}</span>,
  },
  {
    accessorKey: 'enabled',
    header: 'Status',
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Manage organization tenants and Keycloak realms"
        actions={
          <Link href="/tenants/new" className="inline-flex items-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90">
